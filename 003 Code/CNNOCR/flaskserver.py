...
import os
from main import detecttext, recognition
import time
import shutil
...
def ocrPregnantSendDate():
    ...
    if(request.method == 'POST'):
        ...
        for i in range(len(everysundays)):
            ...
            cursor = conn.cursor()
            sql = ''
            sql = "select %s as begindate,%s as enddate, sum(sow_cross) as sow_cross  from ocr_pregnant where DATE(input_date) between  %s "
